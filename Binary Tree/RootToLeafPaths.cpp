...
void solve(Node *node, vector<vector<int>> &ans, vector<int> &ds)
{
    if (isLeaf(node))
    {
        ans.push_back(ds);
    ...
    if (node->left)
        ds.push_back(node->left->data);
    solve(node->left, ans, ds);
    if (node->left)
        ds.pop_back();
    if (node->right)
        ds.push_back(node->right->data);
    solve(node->right, ans, ds);
    if (node->right)
        ds.pop_back();
}
vector<vector<int>> Paths(Node *root)
